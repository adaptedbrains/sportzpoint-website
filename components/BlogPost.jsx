...
const ArticleContent = ({ article, isRelated = false }) => {
  ...
  return (
    <div className="p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <h3 className="font-thin capitalize">{article.author?.name}</h3>
            <p className="text-zinc-500 text-xs">
              {article.published_at_datetime &&
      ...
      <Link
        ...
        className={`${isRelated ? 'text-lg' : 'text-xl'} font-pt-serif font-semibold line-clamp-2 hover:text-green-700 transition-colors`}
      >
        {article.title}
      </Link>
    </div>
...
const RelatedArticleCard = ({ article }) => (
  <div className="px-2">
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <ArticleContent article={article} isRelated={true} />
    </div>
  </div>
);
